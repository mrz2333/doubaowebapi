...
class DoubaoChatClient:
    ...
    def __init__(
        ...
        ms_token: str,
        device_id: str = "000000000000000",
        web_id: str = "0000000000000000000",
        ...
        self.timeout = aiohttp.ClientTimeout(total=None, sock_read=timeout_seconds)
    ...
    async def __aenter__(self) -> "DoubaoChatClient":
        self._session = aiohttp.ClientSession(
            ...
            read_bufsize=2
            ** 20,  # 1MB; default 64KB causes "Chunk too big" for long SSE lines
            headers={
                "User-Agent": (
        ...
        for block in raw.split("\n\n"):
            ...
            if event_name == "gateway-error" and data_str:
                try:
                    ...
                    code, msg = "", data_str
                raise DoubaoChatError(f"gateway-error: code={code} message={msg}")
    ...
    async def chat_stream_samantha(
        ...
        """
        url = f"{self.BASE_URL}/samantha/chat/completion?{urlencode(self._security_params())}"
            ...
            if raw_text.lstrip().startswith("{"):
                try:
                    ...
                    if isinstance(err_obj, dict) and "code" in err_obj:
                        ...
                        msg = err_obj.get("msg") or err_obj.get(
                            "message", raw_text[:200]
                        )
                        raise DoubaoChatError(
    ...
    async def _chat_stream_completion_once(
        ...
        async with self.session.post(
            ...
            if "text/event-stream" not in content_type:
                ...
                raise DoubaoChatError(
                    f"/chat/completion unexpected response (content-type={content_type}): {body_text[:300]}"
                )
            ...
            async for event in iter_sse_events(resp.content):
                ...
                if obj.get("verbose_type"):
                    yield CompletionChunk(
                        tool_info=(
                            f"[{obj['verbose_type']}] {str(obj.get('verbose_payload', ''))[:200]}"
                        ),
                    )
    ...
    async def generate_video(
        ...
        if ref_image_key:
            message["attachments"] = [{"type": "image", "key": ref_image_key}]
    ...
    async def _poll_async_video(
        ...
        params = self._security_params()
        params.pop("fp", None)
        url = f"{self.BASE_URL}/samantha/chat/async/stream?{urlencode(params)}"
        headers = {
            "Accept": "text/event-stream",
            "Content-Type": "application/json",
            "Agw-Js-Conv": "str",
    ...
    def _parse_video_sse(self, raw: str, prompt: str) -> "VideoGenerationResult":
        ...
        for block in raw.split("\n\n"):
            ...
            if event_type == 2005:
                detail = data.get("event_data", "")
                raise DoubaoChatError(f"generate_video error: {str(detail)[:500]}")
            if event_type != 2001:
                continue
    ...
    def _parse_music_response(self, raw: str, prompt: str) -> "MusicGenerationResult":
        ...
        for block in raw.split("\n\n"):
            ...
            if event_type == 2005:
                detail = data.get("event_data", "")
                raise DoubaoChatError(f"generate_music error: {str(detail)[:500]}")
            if event_type != 2001:
                continue
    ...
    async def upload_file(
        ...
        prepare_url = (
            f"{self.BASE_URL}/alice/resource/prepare_upload?{urlencode(params)}"
        )
        async with self.session.post(
            prepare_url,
        ...
        commit_url = f"{self.BASE_URL}/top/v1?Action=CommitImageUpload&Version=2018-08-01&ServiceId={service_id}"
        ...
        async with self.session.post(
            ...
            if not results or results[0].get("UriStatus") != 2000:
                raise DoubaoChatError(f"CommitImageUpload failed: {body}")
    ...
    async def upload_image(
        ...
        upload_url = f"{self.BASE_URL}/samantha/pages/upload_image?{urlencode(params)}"
        ...
        async with aiohttp.ClientSession(
            ...
            async with upload_session.post(upload_url, data=form) as resp:
                if resp.status != 200:
                    raise DoubaoChatError(
                        f"Image upload failed ({resp.status}): {(await resp.text())[:500]}"
                    )
                body = await resp.json()
                if body.get("code") != 0:
                    raise DoubaoChatError(
        ...
        file_url_endpoint = (
            f"{self.BASE_URL}/alice/message/get_file_url?{urlencode(params)}"
        )
        async with self.session.post(
            file_url_endpoint,
            json={
            ...
            if resp.status != 200:
                raise DoubaoChatError(
                    f"get_file_url failed ({resp.status}): {(await resp.text())[:500]}"
                )
            body = await resp.json()
            if body.get("code") != 0:
                raise DoubaoChatError(f"get_file_url error: {body.get('msg', body)}")
            data = body.get("data", {})
    ...
    def from_session(
        ...
        if not cookies.get("sessionid"):
            raise DoubaoChatError(
                f"No sessionid in {session_file}. Use QR login or provide cookies via /v1/session/update."
            )

        # msToken is optional — API works without it, and fake values
        # trigger rate limiting (710022002). Empty string is safest.
        ms_token = cookies.get("msToken", "")
        device_id = params.get("device_id") or "000000000000000"
        web_id = params.get("web_id") or "0000000000000000000"
