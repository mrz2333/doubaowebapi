"""
Tool Calling support for doubaowebapi.
    ...
    for m in _TOOL_CALL_RE.finditer(text):
        ...
        try:
            ...
            try:
                ...
                log.warning(
                    "Failed to parse tool_call JSON even after fix: %s | raw: %s",
                    e,
                    json_str[:200],
                )
                continue
...
def convert_messages_with_tools(
    ...
    for msg in messages:
        ...
        if role == "system":
            ...
            parts.append(TOOL_RESULT_TEMPLATE.format(name=name, content=tool_content))
...
def _drop_old_rounds(
    parts: list[str], max_chars: int, first_user_msg: Optional[str] = None
) -> str:
...
_REFUSAL_PATTERNS = [
    ...
    re.compile(
        r"(?i)i (don'?t|do not) have (the ability|access|permission)", re.DOTALL
    ),
...
def build_continuation_prompt(original_output: str, max_anchor: int = 2000) -> str:
    ...
    """
    anchor = (
        original_output[-max_anchor:]
        if len(original_output) > max_anchor
...
def filter_history_by_topic(
    ...
    if isinstance(prev_text, list):
        prev_text = " ".join(
            p.get("text", "") for p in prev_text if isinstance(p, dict)
        )
    if isinstance(curr_text, list):
        curr_text = " ".join(
            p.get("text", "") for p in curr_text if isinstance(p, dict)
        )

    if detect_topic_change(prev_text, curr_text):
        log.info(
            "Topic change detected (prev=%s... -> new=%s...), trimming history",
            prev_text[:30],
            curr_text[:30],
        )
...
def _select_representative_tools(
    tools: list[dict[str, Any]], max_examples: int = 3
) -> list[dict]:
...
def offload_long_messages(
    parts: list[str], threshold: int = OFFLOAD_MSG_THRESHOLD
) -> list[str]:
